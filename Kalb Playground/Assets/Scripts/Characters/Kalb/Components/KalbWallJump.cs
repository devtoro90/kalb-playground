...
public class KalbWallJump : MonoBehaviour
{
    ...
    /// <summary>
    /// Force disengage wall sliding - used when taking damage
    /// </summary>
    public void ForceDisengageWallSlide()
    {


        // Clear all wall slide states
        isWallSliding = false;
        isStickingToWall = false;
        isTouchingWall = false; // This might be reset by next check, but we want immediate effect
        wallSide = 0;
        awayInputTimer = 0f;
        isPressingAwayFromWall = false;
        currentSlideSpeed = 0f;
        ...
        // Small cooldown to prevent immediate re-engagement
        wallSlideCooldownTimer = WALL_SLIDE_COOLDOWN;
    }
