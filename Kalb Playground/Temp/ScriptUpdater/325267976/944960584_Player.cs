...
public class Player : MonoBehaviour
{
    ...
    public float waterSurfaceOffset = 1.8f; // How much of the face is above water
    ...
    /// <summary>
    /// Handles swimming movement physics without position conflicts
    /// </summary>
    private void HandleSwimMovement()
    {
        ...
        // NORMAL SWIMMING: Use velocity-based approach, not direct position setting
        Vector2 targetVelocity = new Vector2(moveInput.x * currentSwimSpeed, 0);
        
        // Apply buoyancy force instead of setting position
        ApplyBuoyancy();
    ...
    private void ApplyBuoyancy()
    {
        ...
        // Calculate target depth (where neck should be)
        float playerHeight = GetComponent<Collider2D>().bounds.extents.y * 2f;
        float targetDepth = waterSurfaceY - (playerHeight - waterSurfaceOffset);
        
        // Current depth
        float currentDepth = transform.position.y;
        float depthDifference = targetDepth - currentDepth;
        
        // Apply buoyancy force proportional to how far we are from target depth
        // This is more physics-friendly than directly setting position
        float buoyancyForce = depthDifference * 15f; // Strength multiplier
        
        // Add slight damping to prevent oscillation
        float dampingForce = -rb.linearVelocity.y * 2f;
        
        // Apply combined force
        rb.AddForce(new Vector2(0, buoyancyForce + dampingForce));
        
        // Limit maximum buoyancy force
        float maxBuoyancyForce = 5f;
        if (Mathf.Abs(buoyancyForce) > maxBuoyancyForce)
        {
            rb.AddForce(new Vector2(0, Mathf.Sign(buoyancyForce) * maxBuoyancyForce - buoyancyForce));
        }
    }
    ...
    private void SetAnimation(float horizontalInput)
    {
        ...
        else if (isSwimming) // Add swimming animations
        {
            ...
            else if (Mathf.Abs(horizontalInput) > 0.1f)
            {
                if (dashAction.IsPressed() && !isSwimDashing)
                {
                    animator.Play("Player_run"); // Fast swimming
                }
                else
                {
                    animator.Play("Player_walk"); // Normal swimming
                }
            }
            else
            {
                animator.Play("Player_idle"); // Floating in water
            }
        }
        else if (isAttacking)
        {
    ...
    private void CheckWater()
    {
        ...
        // Use the player's collider to check for water overlap
        Collider2D playerCollider = GetComponent<Collider2D>();
        
        // Check if player's collider overlaps with water
        // Use a slightly shrunken area to prevent edge cases
        Vector2 checkCenter = transform.position;
        Vector2 checkSize = playerCollider.bounds.size * 0.8f;
        
        Collider2D waterCollider = Physics2D.OverlapBox(
            checkCenter, 
            checkSize, 
            0f, 
            waterLayer
    ...
    /// <summary>
    /// Gently nudges player toward correct depth (use with buoyancy)
    /// </summary>
    private void AdjustSwimmingPosition()
    {
        if (!isSwimming || currentWaterCollider == null) return;
        
        // Only apply gentle correction if way off
        float playerHeight = GetComponent<Collider2D>().bounds.extents.y * 2f;
        float targetY = waterSurfaceY - (playerHeight - waterSurfaceOffset);
        float currentY = transform.position.y;
        float yDifference = Mathf.Abs(targetY - currentY);
        
        // Only adjust if significantly out of position (> 0.2 units)
        if (yDifference > 0.2f)
        {
            // Very gentle correction
            float newY = Mathf.Lerp(currentY, targetY, Time.deltaTime * 2f);
    ...
    private void OnEnterWater()
    {
        ...
        rb.linearVelocity = new Vector2(
            rb.linearVelocity.x,
            Mathf.Clamp(rb.linearVelocity.y, -2f, 2f) * 0.3f
        );
        
        // Set buoyancy physics
        rb.gravityScale = 0.3f; // Reduced gravity, not zero
        rb.linearDamping = 1f; // Add some drag for water resistance
        ...
        Debug.Log("Entered water - Buoyancy activated");
    }
    ...
    private void OnExitWater()
    {
        ...
        Debug.Log("Exited water - Normal physics restored");
    }
    ...
    void OnDrawGizmosSelected()
    {
        ...
        {
            Gizmos.color = Color.cyan;
            Gizmos.DrawWireSphere(waterCheckPoint.position, waterCheckRadius);
        }
    }
}
