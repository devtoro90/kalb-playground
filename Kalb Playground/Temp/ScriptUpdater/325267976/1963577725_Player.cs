...
public class Player : MonoBehaviour
{
    ...
    public float surfaceOffset = 0.3f; // How much of player is above water
    public float swimJumpForce = 10f;
    public float waterDrag = 3f; // Extra drag in water
    public LayerMask waterLayer;
    public float swimDashSpeed = 12f; // Swimming dash speed
    ...
    private bool isSwimming = false; // Actually swimming at surface
    ...
    void FixedUpdate()
    {
        ...
        if (!isDashing && !isAttacking && !isWallJumping && !isWallSliding && !isHardLanding && !isSwimming)
        {
            HandleFlip();
        }
    }
    ...
    private void HandleDashInput()
    {
        ...
        // Handle swim dash when in water
        if (dashAction.triggered && isSwimming && !isSwimDashing && swimDashCooldownTimer <= 0)
        {
            StartSwimDash();
            return;
        }
        
        // Handle normal dash/run
    ...
    private void HandleJumpInput()
    {
        ...
        // Handle water jump - only when swimming at surface
        if (jumpAction.triggered && isSwimming)
        {
            WaterJump();
            return;
        }
        
        // Handle diving/swimming in water (not jumping)
        if (isInWater && jumpAction.IsPressed())
        {
            // Swimming upward is handled in movement methods
            return;
        }
    ...
    private void HandleMovement()
    {
        ...
        // Handle being in water but not at surface (sinking/rising)
    ...
    /// <summary>
    /// Handles swimming movement at water surface
    /// </summary>
    private void HandleSwimMovement()
    {
        // Horizontal movement in water
        float targetXVelocity = moveInput.x * swimSpeed;
        float velocityDifference = targetXVelocity - rb.linearVelocity.x;
        rb.AddForce(Vector2.right * velocityDifference * swimAcceleration);
        
        // Apply water drag
        rb.linearVelocity = new Vector2(rb.linearVelocity.x * (1f - waterDrag * Time.fixedDeltaTime), 
                                    rb.linearVelocity.y * (1f - waterDrag * Time.fixedDeltaTime));
        
        // Allow diving with down input
        if (moveInput.y < -0.5f && !isSwimDashing)
        {
            rb.AddForce(Vector2.down * swimAcceleration * 2f);
        }
        
        // Allow surfacing with up input
        if (moveInput.y > 0.5f && !isSwimDashing)
        {
            rb.AddForce(Vector2.up * swimAcceleration);
        }
    }

    /// <summary>
    /// Handles movement when in water but not at surface
    /// </summary>
    private void HandleWaterMovement()
    {
        // Reduced gravity movement in water
        float targetXVelocity = moveInput.x * swimSpeed * 0.7f;
        float velocityDifference = targetXVelocity - rb.linearVelocity.x;
        rb.AddForce(Vector2.right * velocityDifference * swimAcceleration * 0.5f);
        ...
        {
            rb.AddForce(Vector2.down * swimAcceleration);
        }
    ...
    {
        if (isDashing || isWallSliding || isHardLanding || isWallJumping || isSwimming || isSwimDashing)
        {
            return;
        }
    ...
    {
        if (Mathf.Abs(moveInput.x) < 0.1f || isHardLanding || isSwimDashing) return;
    ...
    private void WaterJump()
    {
        ...
        Debug.Log("Water jump from surface!");
    ...
    private void SetAnimation(float horizontalInput)
    {
        ...
        else if (isSwimDashing)
        {
            animator.Play("Player_dash"); // Use dash animation for swim dash
        }
        else if (isSwimming)
        {
            ...
            {
                animator.Play("Player_swim_idle");
            }
        }
        else if (isInWater && !isSwimming)
        {
            animator.Play("Player_fall"); // Use fall animation for sinking/rising
        }
        else if (isAttacking)
        {
    ...
    {   
        ...
        Vector2 checkSize = new Vector2(0.8f, 0.2f); // Check at feet level
        Vector2 checkPosition = new Vector2(transform.position.x, transform.position.y - 0.5f);
        Collider2D[] waterColliders = Physics2D.OverlapBoxAll(checkPosition, checkSize, 0f, waterLayer);
        ...
        // Update swimming state if in water
        if (isInWater && waterColliders.Length > 0)
        {
            UpdateWaterSurface(waterColliders[0]);
            UpdateSwimmingState();
        }
        else
        {
    ...
    /// <summary>
    /// Updates the water surface position
    /// </summary>
    private void UpdateWaterSurface(Collider2D waterCollider)
    {
        if (waterCollider == null) return;
        
        // Get the water's surface position
        Bounds waterBounds = waterCollider.bounds;
        waterSurfaceY = waterBounds.max.y;
        
        // Debug visualization
        Debug.DrawLine(
            new Vector3(transform.position.x - 1f, waterSurfaceY, 0),
            new Vector3(transform.position.x + 1f, waterSurfaceY, 0),
            Color.cyan
        );
    }
    ...
    /// </summary>
    private void EnterWater(Collider2D waterCollider)
    {
        ...
        // Apply water physics
        rb.gravityScale = 0.5f; // Reduced gravity in water
        rb.linearDamping = 2f; // Increased drag
        
        // Slow down fall when entering water
        if (rb.linearVelocity.y < 0)
        {
    ...
    /// <summary>
    /// Updates whether player should be swimming at surface
    /// </summary>
    private void UpdateSwimmingState()
    {
        // Player is swimming when in water and near/at surface
        float playerTop = transform.position.y + 0.5f; // Assuming player is 1 unit tall
        float distanceToSurface = waterSurfaceY - playerTop;
        
        // If player is close to surface (within surfaceOffset), they're swimming
        bool shouldBeSwimming = distanceToSurface >= -surfaceOffset && distanceToSurface <= 0.5f;
        ...
        // If swimming, keep player at surface level
        if (isSwimming)
        {
            MaintainSurfacePosition();
        }
    }

    // <summary>
    /// Keeps player at the water surface
    /// </summary>
    private void MaintainSurfacePosition()
    {
        // Target position: head should be at water surface minus offset
        float targetY = waterSurfaceY - surfaceOffset;
        float currentY = transform.position.y;
        
        // If player is below surface, apply upward force
        if (currentY < targetY - 0.1f)
        {
            float force = (targetY - currentY) * 20f;
            rb.AddForce(new Vector2(0, force));
        }
        // If player is above surface, apply downward force
        else if (currentY > targetY + 0.1f)
        {
            float force = (targetY - currentY) * 20f;
            rb.AddForce(new Vector2(0, force));
        }
    ...
    /// <summary>
    /// Starts swimming mode
    /// </summary>
    private void StartSwimming()
    {
        isSwimming = true;
        rb.gravityScale = 0.2f; // Very low gravity when swimming at surface
        
        Debug.Log("Started swimming at surface!");
    }
